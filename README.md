...
| `info.expected` | Expected compiler informational output. Info diagnostics undergo the same grep and sort process as errors. Omit the file when no info is expected. |
...
2. `grep` extracts error, warning, info and hint lines from `compiler.out` into `err.grep`, `warn.grep`, `info.grep` and `hint.grep` respectively.
3. These files are sorted with `sort` (with `LC_COLLATE` set to `C` for stable output) into `err.sort`, `warn.sort`, `info.sort` and `hint.sort`.
4. `diff` compares each `*.sort` to its `*.expected` (`err`, `warn`, `info`, `hint`). Whitespace differences are ignored and carriage returns are stripped.
