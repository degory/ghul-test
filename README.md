...
5. If compilation succeeded, `ghul-runtime.dll` is symlinked into the test directory and the binary is executed via `dotnet`. A `run.in` is written to its standard input and the stream closed. Input and both output streams are carried concurrently, so a program that fills one pipe while still reading another is not deadlocked by the runner. A program that has not exited within 60 seconds is killed and its test fails. Output is captured in `run.out` and compared to `run.expected`.
